...
import { useFireHistory, useSelectFireEvent } from "../state/selectors";
...
const FireHistoryHistogram: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Group fires by month with details
    const monthGroups: Record<string, FireEvent[]> = {};
    ...
    const data: HistogramData[] = Object.entries(monthGroups)
      .map(([label, events]) => ({
        label,
        count: events.length,
  ...
  if (!fireHistory || !histogramData.length) {
    return (
      <div className="panel">
        <h3>📊 Fire History Distribution</h3>
